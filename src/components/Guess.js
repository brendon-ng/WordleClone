  ...
  if (!guessedWord || !complete) {
    ...
    for (let i = 0; i < MAX_WORD_LENGTH; i++) {
      if (_targetWord[i] === _guessedWord[i]) {
        ...
        dispatch(changeKeyColor({letter: _guessedWord[i], color: COLOR_CORRECT}));
        _targetWord[i] = '_';
        _guessedWord[i] = '_';
    ...
    for (let i = 0; i < MAX_WORD_LENGTH; i++) {
      ...
      if (index >= 0) {
        ...
        dispatch(changeKeyColor({letter: _guessedWord[i], color: COLOR_POSITION}));
        _guessedWord[i] = '_';
        _targetWord[index] = '_';
      } else {
        squares[i] = COLOR_WRONG;
        dispatch(changeKeyColor({letter: _guessedWord[i], color: COLOR_WRONG}));
      }
    }
  }
  ...
  squares = squares.map((col, i) => {
    return (
      <View key={i} style={[styles.square, { backgroundColor: col }]}>
        <Text style={[styles.letter, { color: invalid? 'red' : (complete ? 'white' : 'black') }]}>
